...
import 'package:steel_contractor/features/WeldRepair/domain/bloc/weld_repair_event.dart';
import 'package:steel_contractor/features/WeldRepair/domain/bloc/weld_repair_state.dart';

class WeldRepairPage extends StatefulWidget {
  const WeldRepairPage({super.key});

  @override
  State<WeldRepairPage> createState() => _WeldRepairPageState();
}

class _WeldRepairPageState extends State<WeldRepairPage> {
  ScrollController _horizontalScrollController = ScrollController();
  ScrollController _verticalScrollController = ScrollController();

  @override
  void initState() {
    BlocProvider.of<WeldRepairBloc>(
      context,
    ).add(WeldRepairPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(boolLeading: true, title: AppString.weldRepair),
      body: BackgroundWidget(
        child: BlocBuilder<WeldRepairBloc, WeldRepairState>(
          builder: (context, state) {
            if (state is WeldRepairLoadedDataState) {
  ...
  Widget _itemBuilder({required WeldRepairLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required WeldRepairLoadedDataState dataState}) {
  ...
  Widget _dataTableWidget({required WeldRepairLoadedDataState dataState}) {
                                ...
                                if (selected!) {
                                  showCupertinoDialog(
                                    ...
                                    builder: (_) => CustomCupertinoDialog(
                                      onReject: () {
                                        BlocProvider.of<WeldRepairBloc>(context).add(
