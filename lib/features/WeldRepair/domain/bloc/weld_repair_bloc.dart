...
import 'package:steel_contractor/features/WeldRepair/helper/weld_repair_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class WeldRepairBloc
    extends Bloc<WeldRepairEvent, WeldRepairState> {
  WeldRepairBloc() : super(WeldRepairInitial()) {
    on<WeldRepairPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(WeldRepairPageLoadEvent event, emit) async {
    emit(WeldRepairPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _submit({
    ...
    try {
      var res = await WeldRepairHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return WeldRepairHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<WeldRepairState> emit) {
    emit(
