...
class WeldingBloc
    extends Bloc<WeldingEvent, WeldingState> {
  WeldingBloc() : super(WeldingInitial()) {
  ...
  fetchReportActivity({required BuildContext context}) async {
    var res = await WeldingHelper.reportByActivityIdAPI(
      context: context,
    );
    if (res != null) {
  ...
  _eventCompleted(Emitter<WeldingState> emit) {
    emit(
      WeldingLoadedDataState(
        isLoader: isLoader,
