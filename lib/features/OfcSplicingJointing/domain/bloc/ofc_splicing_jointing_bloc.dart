...
import 'package:steel_contractor/features/OfcSplicingJointing/helper/ofc_splicing_jointing_helper.dart';
import 'package:url_launcher/url_launcher.dart';

part 'ofc_splicing_jointing_event.dart';

part 'ofc_splicing_jointing_state.dart';

class OfcSplicingJointingBloc
    extends Bloc<OfcSplicingJointingEvent, OfcSplicingJointingState> {
  OfcSplicingJointingBloc() : super(OfcSplicingJointingInitial()) {
    on<OfcSplicingJointingPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(OfcSplicingJointingPageLoadEvent event, emit) async {
    emit(OfcSplicingJointingPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  loadDataTable({required BuildContext context, emit}) {
    emit(OfcSplicingJointingInitial());
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return OfcSplicingJointingHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<OfcSplicingJointingState> emit) {
    emit(
