...
import 'package:steel_contractor/features/NDTMUT/helper/ndt_mut_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class NDTMUTBloc
    extends Bloc<NDTMUTEvent, NDTMUTState> {
  NDTMUTBloc() : super(NDTMUTInitial()) {
    on<NDTMUTPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(NDTMUTPageLoadEvent event, emit) async {
    emit(NDTMUTPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  fetchReportActivity({required BuildContext context}) async {
    var res = await NDTMUTHelper.reportByActivityIdAPI(
      context: context,
    );
    if (res != null) {
  ...
  _submit({
    ...
    try {
      var res = await NDTMUTHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return NDTMUTHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<NDTMUTState> emit) {
    emit(
      NDTMUTLoadedDataState(
        isLoader: isLoader,
