...
import 'package:steel_contractor/features/NDTMUT/domain/bloc/ndt_mut_event.dart';
import 'package:steel_contractor/features/NDTMUT/domain/bloc/ndt_mut_state.dart';

class NDTMUTPage extends StatefulWidget {
  const NDTMUTPage({super.key});

  @override
  State<NDTMUTPage> createState() => _NDTMUTPageState();
}

class _NDTMUTPageState extends State<NDTMUTPage> {
  ScrollController _horizontalScrollController = ScrollController();
  ScrollController _verticalScrollController = ScrollController();

  @override
  void initState() {
    BlocProvider.of<NDTMUTBloc>(
      context,
    ).add(NDTMUTPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(boolLeading: true, title: AppString.nDTMUT),
      body: BackgroundWidget(
        child: BlocBuilder<NDTMUTBloc, NDTMUTState>(
          builder: (context, state) {
            if (state is NDTMUTLoadedDataState) {
  ...
  Widget _itemBuilder({required NDTMUTLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required NDTMUTLoadedDataState dataState}) {
  ...
  Widget _dataTableWidget({required NDTMUTLoadedDataState dataState}) {
                                ...
                                if (selected!) {
                                  showCupertinoDialog(
                                    ...
                                    builder: (_) => CustomCupertinoDialog(
                                      onReject: () {
                                        BlocProvider.of<NDTMUTBloc>(context).add(
