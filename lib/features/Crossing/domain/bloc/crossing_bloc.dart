...
import 'package:steel_contractor/features/Crossing/domain/bloc/crossing_event.dart';
import 'package:steel_contractor/features/Crossing/domain/bloc/crossing_state.dart';
import 'package:steel_contractor/features/Crossing/helper/crossing_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class CrossingBloc extends Bloc<CrossingEvent, CrossingState> {
  CrossingBloc() : super(CrossingInitial()) {
    on<CrossingPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(CrossingPageLoadEvent event, emit) async {
    emit(CrossingPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _submit({
    ...
    try {
      var res = await CrossingHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return CrossingHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<CrossingState> emit) {
    emit(
      CrossingLoadedDataState(
        isLoader: isLoader,
