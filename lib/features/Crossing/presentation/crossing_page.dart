...
class _CrossingPageState extends State<CrossingPage> {
  ...
  @override
  void initState() {
    BlocProvider.of<CrossingBloc>(
      context,
    ).add(CrossingPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(boolLeading: true, title: AppString.crossing),
      body: BackgroundWidget(
        child: BlocBuilder<CrossingBloc, CrossingState>(
          builder: (context, state) {
            if (state is CrossingLoadedDataState) {
  ...
  Widget _itemBuilder({required CrossingLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required CrossingLoadedDataState dataState}) {
  ...
  Widget _dataTableWidget({required CrossingLoadedDataState dataState}) {
                                ...
                                if (selected!) {
                                  showCupertinoDialog(
                                    ...
                                    builder: (_) => CustomCupertinoDialog(
                                      onReject: () {
                                        BlocProvider.of<CrossingBloc>(context).add(
