...
import 'package:steel_contractor/features/PostHydrotest/helper/post_hydrotest_helper.dart';
import 'package:url_launcher/url_launcher.dart';

part 'post_hydrotest_event.dart';

part 'post_hydrotest_state.dart';

class PostHydrotestBloc extends Bloc<PostHydrotestEvent, PostHydrotestState> {
  PostHydrotestBloc() : super(PostHydrotestInitial()) {
    on<PostHydrotestPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(PostHydrotestPageLoadEvent event, emit) async {
    emit(PostHydrotestPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return PostHydrotestHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<PostHydrotestState> emit) {
    emit(
