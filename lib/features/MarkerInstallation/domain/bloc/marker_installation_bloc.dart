...
import 'package:steel_contractor/features/MarkerInstallation/helper/marker_installation_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class MarkerInstallationBloc
    extends Bloc<MarkerInstallationEvent, MarkerInstallationState> {
  MarkerInstallationBloc() : super(MarkerInstallationInitial()) {
    on<MarkerInstallationPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(MarkerInstallationPageLoadEvent event, emit) async {
    emit(MarkerInstallationPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  fetchReportActivity({required BuildContext context}) async {
    var res = await MarkerInstallationHelper.reportByActivityIdAPI(
      context: context,
    );
    if (res != null) {
  ...
  loadDataTable({required BuildContext context, emit}) {
    emit(MarkerInstallationInitial());
  ...
  _submit({
    ...
    try {
      var res = await MarkerInstallationHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return MarkerInstallationHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<MarkerInstallationState> emit) {
    emit(
      MarkerInstallationLoadedDataState(
        isLoader: isLoader,
