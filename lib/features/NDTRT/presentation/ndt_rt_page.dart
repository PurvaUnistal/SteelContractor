...
class NDTRTPage extends StatefulWidget {
  const NDTRTPage({super.key});

  @override
  State<NDTRTPage> createState() => _NDTRTPageState();
}

class _NDTRTPageState extends State<NDTRTPage> {
  ScrollController _horizontalScrollController = ScrollController();
  ScrollController _verticalScrollController = ScrollController();

  @override
  void initState() {
    BlocProvider.of<NDTRTBloc>(
      context,
    ).add(NDTRTPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(boolLeading: true, title: AppString.nDTRT),
      body: BackgroundWidget(
        child: BlocBuilder<NDTRTBloc, NDTRTState>(
          builder: (context, state) {
            if (state is NDTRTLoadedDataState) {
  ...
  Widget _itemBuilder({required NDTRTLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required NDTRTLoadedDataState dataState}) {
  ...
  Widget _dataTableWidget({required NDTRTLoadedDataState dataState}) {
                                ...
                                if (selected!) {
                                  showCupertinoDialog(
                                    ...
                                    builder: (_) => CustomCupertinoDialog(
                                      onReject: () {
                                        BlocProvider.of<NDTRTBloc>(context).add(
