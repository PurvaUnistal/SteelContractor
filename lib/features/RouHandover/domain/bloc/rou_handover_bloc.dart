...
import 'package:steel_contractor/features/RouHandover/helper/rou_handover_helper.dart';
import 'package:url_launcher/url_launcher.dart';

part 'rou_handover_event.dart';

part 'rou_handover_state.dart';

class RouHandoverBloc extends Bloc<RouHandoverEvent, RouHandoverState> {
  RouHandoverBloc() : super(RouHandoverInitial()) {
    on<RouHandoverPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(RouHandoverPageLoadEvent event, emit) async {
    emit(RouHandoverPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return RouHandoverHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<RouHandoverState> emit) {
    emit(
