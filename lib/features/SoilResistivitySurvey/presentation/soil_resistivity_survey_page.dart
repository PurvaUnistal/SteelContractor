...
class SoilResistivitySurveyPage extends StatefulWidget {
  const SoilResistivitySurveyPage({super.key});

  @override
  State<SoilResistivitySurveyPage> createState() => _SoilResistivitySurveyPageState();
}

class _SoilResistivitySurveyPageState extends State<SoilResistivitySurveyPage> {
  ScrollController _horizontalScrollController = ScrollController();
  ScrollController _verticalScrollController = ScrollController();

  @override
  void initState() {
    BlocProvider.of<SoilResistivitySurveyBloc>(context).add(SoilResistivitySurveyPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBarWidget(
            boolLeading: true,title: AppString.soilResistivitySurvey),
        body: BackgroundWidget(
          child:  BlocBuilder<SoilResistivitySurveyBloc, SoilResistivitySurveyState>(
            builder: (context, state) {
              if (state is SoilResistivitySurveyLoadedDataState) {
  ...
  Widget _itemBuilder({required SoilResistivitySurveyLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required SoilResistivitySurveyLoadedDataState dataState}) {
    return TextFieldWidget(
      ...
      onChanged: (val) {
        BlocProvider.of<SoilResistivitySurveyBloc>(context).add(SearchBpNumberEvent(
          context: context,
          searchBpNumber: val,
        ));
      },
  ...
  Widget _dataTableWidget({required SoilResistivitySurveyLoadedDataState dataState}) {
          ...
          child: Theme(
            ...
            child: Scrollbar(
              ...
              child: SingleChildScrollView(
                ...
                child: Theme(
                  data: Theme.of(context).copyWith(dividerColor:  EnvironmentConfig.of(context)!.primaryTheme,),
                  child:  DataTable(
                    sortAscending: true,
                    columnSpacing: 0,
                    horizontalMargin: 0,
                      ...
                      CommonStyle.dataCheckBox(
                        widget: Checkbox(
                          ...
                          onChanged: (bool? selected) {
                            if (selected!) {
                              showCupertinoDialog(
                                ...
                                builder: (_) => CustomCupertinoDialog(
                                  onReject: () {
                                    BlocProvider.of<SoilResistivitySurveyBloc>(context).add(
                            ...
                            BlocProvider.of<SoilResistivitySurveyBloc>(
                              context,
                            ).add(
                              SelectAllCheckBoxEvent(
                                isSelected: selected ?? false,
                                list: dataState.listOfFilterReportActivity,
                              ),
                            );
                          },
                        ),
                    ...
                    rows: dataState.listOfFilterReportActivity
                      ...
                      return DataRow(
                          cells: <DataCell>[
                            ...
                            DataCell(
                              Checkbox(
                                ...
                                side: WidgetStateBorderSide.resolveWith(
                                      (states) {
                                    return BorderSide(color: EnvironmentConfig.of(context)!.secondaryTheme, width: 2); // when unchecked
                                  },
                                ),
                                value: dataState.selectedRowIds.contains(user.id.toString()),
                                onChanged: (bool? selected) {
                                  BlocProvider.of<SoilResistivitySurveyBloc>(
                                    context,
                                  ).add(
                            ...
                            CommonStyle.dataCell(label: user.reportNo.toString()),
                            CommonStyle.dataCell(label: formattedDate.toString()),
                            CommonStyle.dataCell(label: user.chainageFrom.toString()),
                            CommonStyle.dataCell(label: user.chainageTo.toString()),
                            CommonStyle.dataCell(label: user.spreadName.toString()),
                            CommonStyle.dataCell(label: user.weather.toString()),
                            DataCell(
                            ...
                            CommonStyle.dataCell(label: user.sectionName.toString()),
                          ]);
                    }
                    )
                        .toList(),
