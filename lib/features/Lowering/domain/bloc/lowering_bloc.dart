...
import 'package:steel_contractor/features/Lowering/helper/lowering_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class LoweringBloc
    extends Bloc<LoweringEvent, LoweringState> {
  LoweringBloc() : super(LoweringInitial()) {
    on<LoweringPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(LoweringPageLoadEvent event, emit) async {
    emit(LoweringPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  fetchReportActivity({required BuildContext context}) async {
    var res = await LoweringHelper.reportByActivityIdAPI(
      context: context,
    );
    if (res != null) {
  ...
  _submit({
    ...
    try {
      var res = await LoweringHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return LoweringHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<LoweringState> emit) {
    emit(
      LoweringLoadedDataState(
        isLoader: isLoader,
