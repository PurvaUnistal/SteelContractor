...
import 'package:steel_contractor/features/NDTLPT/helper/ndt_lpt_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class NDTLPTBloc
    extends Bloc<NDTLPTEvent, NDTLPTState> {
  NDTLPTBloc() : super(NDTLPTInitial()) {
    on<NDTLPTPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(NDTLPTPageLoadEvent event, emit) async {
    emit(NDTLPTPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _submit({
    ...
    try {
      var res = await NDTLPTHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return NDTLPTHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<NDTLPTState> emit) {
    emit(
