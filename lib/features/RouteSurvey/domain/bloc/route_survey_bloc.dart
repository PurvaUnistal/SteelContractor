...
import 'package:steel_contractor/features/RouteSurvey/helper/route_survey_helper.dart';
import 'package:url_launcher/url_launcher.dart';

part 'route_survey_event.dart';

part 'route_survey_state.dart';

class RouteSurveyBloc extends Bloc<RouteSurveyEvent, RouteSurveyState> {
  RouteSurveyBloc() : super(RouteSurveyInitial()) {
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return RouteSurveyHelper.showConfirmationDialog(
      ...
      onConfirm: () async {
        isBtnLoader = true;
        _eventCompleted(emit);
        await _submit(status: "1", remark: "", context: event.context);
        isBtnLoader = false;
        _eventCompleted(emit);
      },
    );
  ...
  _eventCompleted(Emitter<RouteSurveyState> emit) {
    emit(
