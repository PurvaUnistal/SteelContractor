...
class RouteSurveyPage extends StatefulWidget {
  const RouteSurveyPage({super.key});
...
class _RouteSurveyPageState extends State<RouteSurveyPage> {
  ...
  void initState() {
    BlocProvider.of<RouteSurveyBloc>(
      context,
    ).add(RouteSurveyPageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(boolLeading: true, title: AppString.routeSurvey),
      body: BackgroundWidget(
        child: BlocBuilder<RouteSurveyBloc, RouteSurveyState>(
          builder: (context, state) {
            if (state is RouteSurveyLoadedDataState) {
  ...
  Widget _itemBuilder({required RouteSurveyLoadedDataState dataState}) {
    return Column(
      children: [
        Padding(
  ...
  Widget _searchTextField({required RouteSurveyLoadedDataState dataState}) {
  ...
  Widget _dataTableWidget({required RouteSurveyLoadedDataState dataState}) {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Theme(
                      ...
                      child: DataTable(
                                ...
                                if (selected!) {
                                  showCupertinoDialog(
                                    ...
                                    builder: (_) => CustomCupertinoDialog(
                                          onReject: () {
                                            BlocProvider.of<RouteSurveyBloc>(context).add(ActivityRejectEvent(context: context));
                                          },
                                          onApprove: () {
                                            BlocProvider.of<RouteSurveyBloc>(context).add(ActivityApprovedEvent(context: context));
                                          },
                                        ),
                                  );
                                }
                        ...
                        rows:
                            dataState.listOfFilterReportActivity.mapIndexed((
                              index,
                              user,
                            ) {
                              ...
                              return DataRow(
                                cells: <DataCell>[
                                  CommonStyle.dataCell(label: (dataState.listOfFilterReportActivity.indexOf(user) + 1 + (dataState.pageNo - 1) * 10)
                                            .toString(),
                                  ),
                                  DataCell(
                              ...
                              fontSize: 8
                                  ...
                                  CommonStyle.dataCell(
                                    label: user.spreadName.toString(),
                                  ),
                                  CommonStyle.dataCell(
