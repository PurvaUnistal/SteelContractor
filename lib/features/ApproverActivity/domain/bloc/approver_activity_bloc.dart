...
import 'package:steel_contractor/features/ApproverActivity/presentation/widgets/confirmation_widget.dart';
...
import 'approver_activity_event.dart';
import 'approver_activity_state.dart';

class ApproverActivityBloc
    extends Bloc<ApproverActivityEvent, ApproverActivityState> {
  ApproverActivityBloc() : super(ApproverActivityInitial()) {
    on<ApproverActivityPageLoadEvent>(_pageLoad);
  ...
  _searchBpNumber(SearchBpNumberEvent event, emit) async {
    ...
    if (event.searchBpNumber.isNotEmpty) {
      listOfFilterReportActivity = listOfReportActivity.where((e)=> e.reportNo.toString().contains(query)).toList();
    } else {
      listOfFilterReportActivity = listOfReportActivity;
    }
    _eventCompleted(emit);
  ...
  _eventCompleted(Emitter<ApproverActivityState> emit) {
    emit(
      ApproverActivityLoadedDataState(
        isLoader: isLoader,
