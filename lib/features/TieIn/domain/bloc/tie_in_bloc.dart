...
import 'package:steel_contractor/features/TieIn/helper/tie_in_helper.dart';
import 'package:url_launcher/url_launcher.dart';

class TieInBloc
    extends Bloc<TieInEvent, TieInState> {
  TieInBloc() : super(TieInInitial()) {
    on<TieInPageLoadEvent>(_pageLoad);
  ...
  _pageLoad(TieInPageLoadEvent event, emit) async {
    emit(TieInPageLoadState());
    isLoader = false;
    isBtnLoader = false;
    pageNo = 1;
  ...
  _submit({
    ...
    try {
      var res = await TieInHelper.saveActivityApproveReject(
        context: context,
        status: status,
        remark: remark,
      );
  ...
  _activityApproved(ActivityApprovedEvent event, emit) async {
    return TieInHelper.showConfirmationDialog(
  ...
  _eventCompleted(Emitter<TieInState> emit) {
    emit(
      TieInLoadedDataState(
        isLoader: isLoader,
